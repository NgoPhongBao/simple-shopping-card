...
const Permission = () => {
  ...
  useEffect(() => {
    getListPermission(query);
  }, []);

  const getListPermission = async (query) => {
    setIsLoading(true);
    try {
      const res = await api.get("/permission", query);
      const { data = {} } = res;
    ...
    <div className="m-4">
      <div className="my-4">
        <Breadcrumb>
          ...
          <Breadcrumb.Item>Danh sách quyền</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      <div className=" h-full fadein">
        <div className="bg-white mt-0 shadow-md rounded-md overflow-hidden">
          <Search query={query} setQuery={setQuery} getListPermission={getListPermission} />
        </div>
        <div className="mt-4 mb-1">
          <Link to="/permission/add" className="inline-block">
            <Button
              type="primary"
